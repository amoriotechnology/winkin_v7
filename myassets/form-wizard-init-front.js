...
function validateWizard(wizard, steps) {
	...
	switch (steps) {
    	case 0:
    		var serv = $("input[type=radio][name='court']:checked").val();
	    	if(serv === undefined) {
	    		$('.choose-serv-error-msg').focus();
	    		$('.choose-serv-error-msg').html('<div class="alert alert-danger alert-dismissible fade show"> <b>Please choose any one court</b> <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"><i class="bi bi-x"></i></button> </div>');
